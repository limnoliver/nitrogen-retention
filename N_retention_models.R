...
png("E:/Dropbox/FLAME_MississippiRiver/N_Model/N_retention_Drivers.png", res=400, width=4.2,height=6, units="in")
...
par(mgp=c(3,.5,0))
par(tck=-0.03)
pep_col<-add.alpha('lightskyblue1', 1)

plot(Gooddata$RNO3~ Gooddata$TotalArea, las=1, pch=16, cex.axis=cex, cex=cexpt)
points(Gooddata$RNO3[Gooddata$Pool=='Pepin']~ Gooddata$TotalArea[Gooddata$Pool=='Pepin'], cex=cexpt, pch=21, col='black', bg=pep_col)
mtext(expression(paste('Total Area (', 'km'^'2', ')')),1,2, cex=cex)
axis(2, labels=NA, cex=cex)
abline(h=0)

# text(52,0.45, 'Lake Pepin')
# arrows(75,0.45,87,0.47, length=0.07)
# legend('topleft',  c('Lake Pepin', 'Other Pools'), pch=c(21, 16), col='black', bty='o', bg='grey90', pt.cex=cexpt, pt.bg='white', box.col=NA)
# 
legend('topleft',  c('Lake Pepin', 'Other Pools'), pch=c(21, 16), col='black', bty='o', pt.cex=cexpt, pt.bg=pep_col, box.col=NA)
...
points(Gooddata$RNO3[Gooddata$Pool=='Pepin']~ Gooddata$NO3_start[Gooddata$Pool=='Pepin'], cex=cexpt, pch=21, col='black', bg=pep_col)
axis(2, labels=NA)
abline(h=0)
mtext(expression(paste('Incoming ', NO[3], " (mg N L"^"-1", ")")),1,2, cex=cex)

plot(Gooddata$RNO3~ Gooddata$RTurb, yaxt="n", pch=16, cex.axis=cex, cex=cexpt)
points(Gooddata$RNO3[Gooddata$Pool=='Pepin']~ Gooddata$RTurb[Gooddata$Pool=='Pepin'], cex=cexpt, pch=21, col='black', bg=pep_col)
mtext(expression(paste('Turbidity Removal (%)')),1,2, cex=cex)
axis(2, labels=NA, cex=cex)
abline(h=0)

plot(Gooddata$RNO3~ Gooddata$H, las=1, pch=16, cex.axis=cex, cex=cexpt)
points(Gooddata$RNO3[Gooddata$Pool=='Pepin']~ Gooddata$H[Gooddata$Pool=='Pepin'], cex=cexpt, pch=21, col='black', bg=pep_col)
...
points(Gooddata$RNO3[Gooddata$Pool=='Pepin']~ Gooddata$WRT_d[Gooddata$Pool=='Pepin'], cex=cexpt, pch=21, col='black', bg=pep_col)
...
pdf("E:/Dropbox/FLAME_MississippiRiver/N_retention_PerPool_vertical_PNAS.pdf", width=3.42,height=2.5197)
# png("E:/Dropbox/FLAME_MississippiRiver/N_retention_PerPool_vertical_PNAS.png", res=600, width=3.42,height=2.5197, units="in")
